...
class ModelInvertedPendulumWalker(ModelBase):
    def __init__(self):
    ...
    def dynamics(self, t, state, params: dict | None = None):
        ...
        # logger.info(f"ModelInvertedPendulumWalker: dynamics() called with t={t}, state={state}, torque={torque}")

        theta = state[0]
        theta_dot = state[1]

        theta_ddot = (g * np.sin(theta) * length + (torque / mass)) / (length**2 + 1e-6)  # add small term to avoid division by zero
    ...
    def visualize(
        ...
        *,
        ...
        if show_swing:
            swing_angle = theta - 2 * angle_of_attack # type: ignore
